/* FunPay Vertex database backup tool. */
...
static void fpv_db_backup_usage(const char* name) {
  fprintf(
      stderr,
      "Usage: %s --output <path> [--db-url <url>] [--data-dir <path>]\n"
      "Environment:\n"
      "  FPV_DB_URL  Database URL (postgres:// or sqlite://).\n",
      name ? name : "fpv_db_backup");
}
...
int main(int argc, char* argv[]) {
  ...
  if (db_url && fpv_is_postgres_url(db_url)) {
    char* args[] = {
        ...
        (char*)db_url,
        NULL};
    int rc = fpv_spawn("pg_dump", args);
    if (rc != 0) {
      fprintf(stderr, "pg_dump failed with code %d.\n", rc);
      return 1;
    }
  ...
  result = fpv_db_backup(db, output);
  if (result != FPV_OK) {
    fprintf(stderr, "Backup failed (code %d): %s\n", result, fpv_db_error(db));
    fpv_db_close(db);
    return 1;
  }
