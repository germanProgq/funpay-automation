...
static const char* fpv_tg_cbt_auth_login = "auth_login";
static const char* fpv_tg_cbt_auth_join = "auth_join";
static const char* fpv_tg_cbt_auth_create = "auth_create";
static const char* fpv_tg_cbt_auth_timezone = "auth_tz";
static const char* fpv_tg_cbt_auth_timezone_custom = "auth_tz_custom";
static const char* fpv_tg_cbt_auth_currency = "auth_cur";
static const char* fpv_tg_cbt_auth_currency_custom = "auth_cur_custom";
...
static fpv_tg_session_t* fpv_tg_find_session(
    fpv_telegram_service_t* service,
    int64_t user_id) {
...
static const char* fpv_tg_get_session_user_id(
    const fpv_telegram_service_t* service,
    int64_t user_id) {
  if (!service || user_id == 0) {
    return NULL;
...
static fpv_result_t fpv_tg_load_sessions(fpv_telegram_service_t* service) {
  if (!service || !service->storage.cache_dir) {
    return FPV_ERR_INVALID_ARGUMENT;
  }
...
static fpv_result_t fpv_tg_save_sessions(fpv_telegram_service_t* service) {
  if (!service || !service->storage.cache_dir) {
    return FPV_ERR_INVALID_ARGUMENT;
  }
  char* path = fpv_path_join(service->storage.cache_dir, "tg_sessions.json");
...
static void fpv_tg_state_data_clear(fpv_tg_state_data_t* data) {
  if (!data) {
    return;
  }
...
static void fpv_tg_state_data_copy(
...
static bool fpv_tg_set_session(
    fpv_telegram_service_t* service,
    int64_t tg_user_id,
    const char* user_id) {
...
static char* fpv_tg_normalize_email(const char* email) {
...
static void fpv_tg_uppercase_ascii(char* text) {
  if (!text) {
    return;
  }
  for (char* ptr = text; *ptr; ptr++) {
...
static void fpv_tg_send_auth_error(
    fpv_telegram_service_t* service,
    int64_t chat_id,
    fpv_result_t result) {
  if (!service || chat_id == 0) {
...
static fpv_result_t fpv_tg_resolve_user_context(
    fpv_telegram_service_t* service,
    const char* user_id,
...
static char* fpv_tg_build_auth_menu_keyboard(fpv_telegram_service_t* service) {
  fpv_tg_keyboard_t* kb = fpv_tg_keyboard_create(true, false);
  if (!kb) {
    return NULL;
  }
...
static char* fpv_tg_build_workspace_menu_keyboard(fpv_telegram_service_t* service) {
  fpv_tg_keyboard_t* kb = fpv_tg_keyboard_create(true, false);
  if (!kb) {
    return NULL;
  }
...
static char* fpv_tg_build_timezone_keyboard(fpv_telegram_service_t* service) {
  fpv_tg_keyboard_t* kb = fpv_tg_keyboard_create(true, false);
  if (!kb) {
    return NULL;
  }
...
static char* fpv_tg_build_currency_keyboard(fpv_telegram_service_t* service) {
  fpv_tg_keyboard_t* kb = fpv_tg_keyboard_create(true, false);
  if (!kb) {
    return NULL;
  }
...
static void fpv_tg_send_login_menu(fpv_telegram_service_t* service, int64_t chat_id) {
  if (!service || chat_id == 0) {
    return;
  }
  if (!service->identity) {
...
static void fpv_tg_send_workspace_menu(fpv_telegram_service_t* service, int64_t chat_id) {
  if (!service || chat_id == 0) {
    return;
  }
  if (!service->identity) {
...
static void fpv_tg_finalize_workspace_create(
    fpv_telegram_service_t* service,
    int64_t chat_id,
    int64_t tg_user_id,
    const char* org_name,
...
static bool fpv_tg_handle_auth_state(
    fpv_telegram_service_t* service,
    const fpv_tg_message_t* message) {
  if (!service || !message) {
    return false;
...
static bool fpv_tg_handle_auth_callback(
    fpv_telegram_service_t* service,
    const fpv_tg_callback_t* callback) {
